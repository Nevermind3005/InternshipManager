...
const handlerApproveInternship = async (id: string): Promise<IInternshipRes> => {
    return await authHttpClient
        .post(API.Endpoints.Internship.HandlerApprove(id))
        .json<IInternshipRes>();
};

export const useHandlerApproveInternship = (
    ...
    return useMutation<IInternshipRes, Error, string>({
        ...restOptions,
        mutationFn: (id: string) => handlerApproveInternship(id),
