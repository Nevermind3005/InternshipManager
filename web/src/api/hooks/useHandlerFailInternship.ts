...
const handlerFailInternship = async (id: string): Promise<IInternshipRes> => {
    return await authHttpClient
        .post(API.Endpoints.Internship.HandlerFail(id))
        .json<IInternshipRes>();
};

export const useHandlerFailInternship = (
    ...
    return useMutation<IInternshipRes, Error, string>({
        ...restOptions,
        mutationFn: (id: string) => handlerFailInternship(id),
